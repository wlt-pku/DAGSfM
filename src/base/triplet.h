...
struct Triplet {
   ...
   */
  Triplet(IndexT ii, IndexT jj, IndexT kk) : i(ii), j(jj), k(kk) {}
  ...
  bool contain(const std::pair<IndexT, IndexT>& edge) const {
    ...
    if ((It == i || It == j || It == k) && (Jt == i || Jt == j || Jt == k) &&
        It != Jt) {
      return true;
    } else {
      return false;
...
void TripletListing(std::map<image_pair_t, TwoViewGeometry>& EpipolarGeometry,
                    std::vector<Triplet>& triplets,
                    std::map<image_pair_t, double>& EG_ratio,
                    std::map<image_pair_t, int>& EdgeVotes) {
  ...
  for (const auto& epi : EpipolarGeometry) {
    ...
    if (J > maxNodeIndex) maxNodeIndex = J;
  }
  maxNodeIndex += 1;
  ...
  LOG(INFO) << "creating adjacent matrix, with " << maxNodeIndex << " nodes"
            << std::endl;
  ...
  for (const auto& tri : triplets) {
    ...
    if (EpipolarGeometry.count(ij) > 0 && EpipolarGeometry.count(ik) > 0 &&
        EpipolarGeometry.count(jk) > 0) {
      Eigen::Matrix3d Rij =
          QuaternionToRotationMatrix(EpipolarGeometry[ij].qvec);
      Eigen::Matrix3d Rik =
          QuaternionToRotationMatrix(EpipolarGeometry[ik].qvec);
      Eigen::Matrix3d Rjk =
          QuaternionToRotationMatrix(EpipolarGeometry[jk].qvec);
      Eigen::Matrix3d Rcycle = Rij * Rjk * Rik.transpose();

      double angleError = ComputeRotationError(Rcycle, Identity);
      // outTri << tri.i << " " << tri.j << " " << tri.k << " " << angleError <<
      // std::endl;
      EdgeVotes[ij]++;
      EdgeVotes[jk]++;
      EdgeVotes[ik]++;
