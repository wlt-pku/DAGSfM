...
// alignment.
class DistributedMapperController : public Thread, public SfMMaster {
 public:
  struct Options {
 ...
 private:
  ...
  void LoadImages();
  void LoadTwoviewGeometries();

  // Build consistent tracks across images and features.
  void BuildTracks();
  ...
  MirrorSimilarityGraph similarity_graph_;
  ...
  // track elements.
  std::vector<TrackElement> track_elements_;
  std::vector<std::pair<track_t, track_t>> track_pairs_;
  std::unordered_map<track_t, std::vector<TrackElement>> tracks_;
};
